...
ReturnStatus ClientApplication::Init() noexcept {
  timer_.Init();
...
void ClientApplication::HandlePlayerTurn() {
...
void ClientApplication::CheckEndTurnCondition() {
...
void ClientApplication::DrawTexts() {
...
void ClientApplication::Deinit() noexcept { world_.Deinit(); }

void ClientApplication::CreateBalls() noexcept {
...
void ClientApplication::CreateWalls() noexcept {
...
void ClientApplication::CreateHoles() noexcept {
  constexpr auto win_size_meter =
      Metrics::PixelsToMeters(Math::Vec2F(kWindowWidth_, kWindowHeight_));
...
void ClientApplication::UpdateScores() noexcept {
...
void ClientApplication::DrawBalls() {
  int i = 0;
  for (const auto& col_ref : ball_collider_refs_) {
    const auto& col = world_.GetCollider(col_ref);
    if (!col.Enabled()) {
...
void ClientApplication::DrawWalls() {
...
void ClientApplication::DrawHoles() {
...
void ClientApplication::OnTriggerEnter(PhysicsEngine::ColliderRef colliderRefA,
